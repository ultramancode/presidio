...
class NoOpNlpEngine(NlpEngine):
    ...
    engine_name = "no_op"
    is_available = True
    _SUPPORTED_MODEL_KEYS = {"lang_code", "model_name"}
    ...
    def _validate_model_params(cls, model: Dict) -> Dict[str, str]:
        ...
        unsupported_keys = set(model.keys()) - cls._SUPPORTED_MODEL_KEYS
        if unsupported_keys:
            raise ValueError(
                "NoOpNlpEngine model configuration does not support these keys: "
        ...
        if model["model_name"] != cls.engine_name:
            raise ValueError(
                f"NoOpNlpEngine model_name must be '{cls.engine_name}', "
        ...
        **kwargs,
        ...
        for item in texts:
            if as_tuples:
                ...
                text, context = item
                text = self._validate_string(text, "text")
                yield text, self._create_empty_nlp_artifacts(language), context
            else:
                text = self._validate_string(item, "text")
