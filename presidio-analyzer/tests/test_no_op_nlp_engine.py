...
class TestNoOpNlpEngine:
    ...
    def test_when_init_with_non_no_op_model_name_then_raises(self):
        with pytest.raises(ValueError, match="model_name must be 'no_op'"):
            NoOpNlpEngine(models=[{"lang_code": "en", "model_name": "en_core_web_lg"}])

    def test_when_init_with_unsupported_model_key_then_raises(self):
    ...
    def test_when_load_fails_then_engine_is_not_marked_loaded(self):
        engine = NoOpNlpEngine(models=[{"lang_code": "en", "model_name": "no_op"}])
    ...
    def test_when_process_batch_with_non_string_text_then_raises(
        self, no_op_nlp_engine
    ):
        with pytest.raises(TypeError, match="text must be a string"):
            list(no_op_nlp_engine.process_batch([123], language="en"))

    def test_when_process_batch_as_tuples_with_non_string_text_then_raises(
        self, no_op_nlp_engine
    ):
        with pytest.raises(TypeError, match="text must be a string"):
            list(
...
class TestNoOpNlpEngineProvider:
    ...
    def test_when_no_op_configuration_has_unsupported_key_then_raises(self):
        nlp_configuration = {
            "nlp_engine_name": "no_op",
            "models": [{"lang_code": "en", "model_name": "no_op"}],
